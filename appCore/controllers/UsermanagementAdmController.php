...
class UsermanagementAdmController extends AdmController {
    ...
    protected function _getSessionValue($index, $default = false) {
        if (!isset($_SESSION[$this->sessionPrefix][$index]))
            $_SESSION[$this->sessionPrefix][$index] = $default;
    ...
    protected function _getErrorMessage($code) {
        ...
        switch ($code) {
            case "no permission": $message = "You don't have permission to do this.";
                break;
            case "cannot self delete": $message = "";
                break;
            case "password mismatch": $message = Lang::t('PASSWRONG', 'register');
            //...
            case "": $message = "";
                break;
            default: $message = Lang::t('_OPERATION_FAILURE', 'standard');
                break;
        }
    ...
    public function show() {
        ...
        switch ($res) {
            case 'ok_assignuser': $message = getResultUi(Lang::t('_OPERATION_SUCCESSFUL', 'standard'));
                break;
            case 'err_assignuser': $message = getErrorUi(Lang::t('_GROUP_USERASSIGN_ERROR', 'admin_directory'));
                break;
            default: $message = "";
        }
        ...
        if (is_array($list) && count($list) > 0) {
            ...
            foreach ($list as $idst => $record) {
                $record_row = array(
                    ...
                    'lastname' => Layout::highlight($record['lastname'], $filter_text) . ' ' . Layout::highlight($record['firstname'], $filter_text),
    ...
    function createuser_action() {
        ...
        $query = "SELECT COUNT(*) FROM `core_user` WHERE valid = '1'";
        $rs = sql_query($query);
    ...
    function moduser_action() {
        ...
        if ($check_pwd && !Get::cfg('demo_mode'))
    ...
    protected function _getNodeActions($node) {
        ...
        if (!is_array($node))
            return false; //unrecognized type for node data
        $actions = array();
        $id_action = $node['id'];
    ...
    function assignuser() {
        ...
        if ($id > 0) {
            ...
            if ($cancel) {
                ...
                $selector->loadSelector(Util::str_replace_once('&', '&amp;', $jump_url), array('index.php?r=' . $this->link . '/show' => Lang::t('_ORG_CHART', 'organization_chart'),
                    Lang::t('_ASSIGN_USERS', 'organization_chart')), false, true);
            }
        } else {
            Util::jump_to($back_url);
        }
            ...
            if (isset($fields_use[$id_field])) {
                ...
                foreach ($arr_idgroups as $idst_group) {
                    $res = $fl->addFieldToGroup(
                            $id_field, $idst_group, isset($fields_mandatory[$id_field]) ? 'true' : 'false', isset($fields_invisible[$id_field]) ? 'readwrite' : 'readonly', isset($fields_userinherit[$id_field]) ? 1 : 0
                    );
                    if ($res)
                        $count++;
                }
    ...
    public function importusers() {
        ...
        switch ($step) {
            ...
            case 2: {
                    ...
                    require_once(_lms_ . '/modules/quest_bank/PHPExcel/PHPExcel.php');
                    ...
                    switch ($separator_info) {
                        case "comma": $separator = ",";
                            break;
                        case "dotcomma": $separator = ";";
                            break;
                        case "manual": $separator = Get::req('import_separator_manual', DOTY_STRING, "");
                            break;
                    }
...
//                    echo '</pre>';
            ...
            case 3: {
                    ...
                    if (count($results) > 1) {
                        ...
                        $table = new Table(
                                Get::sett('visuItem', 25), Lang::t('_ERRORS', 'admin_directory'), Lang::t('_ERRORS', 'admin_directory')
                        );
            ...
            foreach ($finfo as $id_field => $data) {
                ...
                if (isset($input_data[$id_field])) {
                    ...
                    switch ($data[FIELD_INFO_TYPE]) {
                        case "": $value_to_set = Format::dateDb($input_data[$id_field], 'date');
                            break;
