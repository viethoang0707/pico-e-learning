...
class Module_Organization extends LmsModule {
    ...
    function loadBody() {
        ...
        if ($GLOBALS['op'] == 'custom_playitem') {
            ...
            error_log($id_item);
            if (!$id_item) {
                $query = "SELECT * FROM " . $GLOBALS['prefix_lms'] . "_organization" . " WHERE objectType = '" . 'test' . "'" . " AND idCourse = " . $_SESSION['idCourse'] . "";
                $res = sql_query($query);
                $org = mysql_fetch_array($res);
                if ($org) {
                    $id_item = $org['idOrg'];
                }
                error_log($query);
            ...
            /* Thay đổi xem file pdf ở đâu */

            if ($folder->otherValues[REPOFIELDOBJECTTYPE] != 'item') {
