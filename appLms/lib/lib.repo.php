...
class RepoTreeView extends TreeView {
    ...
    function _getOpPropertiesId() {
        return $this->id . '_properties_';
    }

    function _getOpCreateLO() {
        return $this->id . '_createLOBegin';
    }

    function _getOpCreateLOSel() {
        return $this->id . '_createLOSel';
    }

    function _getOpCreateLOEnd() {
        return $this->id . '_createLOEnd';
    }

    function _getOpEditLO() {
        return $this->id . '_editLOBegin';
    }

    function _getOpEditLOEnd() {
        return $this->id . '_editLOEnd';
    }

    function _getOpEditLOId() {
        return $this->id . '_editLOBegin';
    }

    function _getOpCopyLO() {
        return $this->id . '_copyLOBegin';
    }

    function _getOpCopyLOSel() {
        return 'copyLOSel';
    }

    function _getOpCopyLOEndOk() {
        return $this->id . '_copyLOEndOk';
    }

    function _getOpCopyLOEndCancel() {
        return $this->id . '_copyLOEndCancel';
    }

    function _getOpCopyLOId() {
        return $this->id . '_copyLOBegin';
    }

    function _getOpPlayEnd() {
        return $this->id . '_playEnd';
    }

    function _getSaveImage() {
        return getPathImage() . 'treeview/save.gif';
    }

    function _getOpPropertiesImg() {
    ...
    function _getCreateLOLabel() {
        return Lang::t('_REPOCREATELO', 'storage');
    }

    function _getCreateLOAlt() {
        return Lang::t('_REPOCREATELO', 'storage');
    }

    function _getCreateLOImage() {
    ...
    function _getOpPropertiesTitle() {
    ...
    function extendedParsing($arrayState, $arrayExpand, $arrayCompress) {
        ...
        if (isset($arrayState[$this->_getOpCreateLO()])) {
            ...
            if (isset($_GET['create_result'])) {
                if ($_GET['create_result'] == '1') {
                    ...
                    $this->tdb->addItem($this->getSelectedFolderId(), $lo->getTitle(), $this->creatingObjectType, $idResource, 0, /* idCategory */ 0, /* idUser */ getLogUserId(), /* idAuthor */ '1.0' /* version */, '_DIFFICULT_MEDIUM', /* difficult */ '', /* description */ '', /* language */ '', /* resource */ '', /* objective */ date("Y-m-d H:i:s"));
                    $this->refresh = TRUE;
        ...
        if (isset($arrayState[$this->_getOpCopyLOEndOk()])) {
            ...
            if ($saveObj->nameExists($saveName)) {
                ...
                if ($idResource != 0) {
                    $lo = createLO($saveData['objectType']);
                    $this->tdb->addItem($this->getSelectedFolderId(), $saveData['name'], $saveData['objectType'], $idResource, 0, /* idCategory */ 0, /* idUser */ getLogUserId(), /* idAuthor */ '1.0' /* version */, '_DIFFICULT_MEDIUM', /* difficult */ '', /* description */ '', /* language */ '', /* resource */ '', /* objective */ date("Y-m-d H:i:s"));
                    $this->refresh = TRUE;
                }
            }
            $this->op = 'copyLOEndOk';
    ...
    function LOSelector($module, $back_url) {
        ...
        while (list($objectType) = sql_fetch_row($rs)) {
            if( $objectType == 'htmlpage' || $objectType == 'item' || $objectType  == 'test' || $objectType  == 'poll') { //change option of radio in database table learning_lo_types 
