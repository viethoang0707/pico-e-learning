...
class CoursepathLms extends Model {
    ...
    public function getUserStartedCoursepath($id_user, $conditions = '') {
        ...
        while ($o = sql_fetch_object($result)) {
            ...
            if ($o->courses <= $res[$o->id_path]['course_completed'])
                unset($res[$o->id_path]);
            else {
    ...
    public function getPathOfItem($id_item) {
        $sql_1 = "SELECT idResource FROM learning_organization WHERE idOrg = " . $id_item;
        $rs_1 = sql_query($sql_1);
        list($idResource) = sql_fetch_row($rs_1);
        $sql_2 = "SELECT path FROM learning_materials_lesson WHERE idLesson = " . $idResource;
        $rs_2 = sql_query($sql_2);
        list($fileName) = sql_fetch_row($rs_2);
