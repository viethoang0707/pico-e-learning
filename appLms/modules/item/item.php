...
// XXX: additem
    ...
    function readPDF() {
        $fileName = getNamePdf($_GET['id_item']);
        //$urlApi = "http://trananhapi.vietinterview.com/pdfjs/web";
        $urlApi = "https://trananh.vietinterview.com/files/appLms/item/";
        $GLOBALS['page']->add(
        '<div class="std_block">'
                //.'<iframe style="width: 100%; height:500px;" src="'.$urlApi.'/viewer.html?name='.$fileName.'">'
		.'<iframe style="width: 100%; height:500px;" src="'.$urlApi.$fileName.'#toolbar=0">'
                .'</iframe>'
        . '</div>', 'content');
    }

    function getNamePdf($id_item) {
        $course = new CoursepathLms();
        $fileName = $course->getPathOfItem($id_item);
        return $fileName;
    }
