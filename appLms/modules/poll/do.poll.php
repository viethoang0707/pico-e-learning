...
function play($object_poll, $id_param) {
    ...
    if ($previous_page === false) {
        ...
        if (isset($_POST['next_page']))
            ++$page_to_display;
        if (isset($_POST['prev_page']) && $page_to_display > 1)
            --$page_to_display;
    }
    ...
    while (list($idQuest, $type_quest, $type_file, $type_class) = sql_fetch_row($re_question)) {
        ...
        $GLOBALS['page']->add($quest_obj->play($quest_sequence_number, false, $id_track, false), 'content');
...
function writePollReport($id_poll, $id_param, $back_url, $mvc = false) {
    ...
    if (sql_num_rows($res) > 0) {
        ...
        $str = Form::openForm('tree_filter_form', 'index.php?modname=stats&amp;op=statcourse')
                ...
                . Form::getDropdown(Lang::t('_FILTEREDITIONSELECTTITLE', 'stats', 'lms'), 'poll_editions_filter', 'poll_editions_filter', $arr_editions, $editions_filter)
...
//            echo $idQuest;
//            echo "<br/>";
//            $filetext.=$quest_obj->export_CSV_Poll($quest_sequence_number, $tot_tracks, $valid_track);
//            $filetext .= "\r\n";
        } else {
        ...
        if (($type_quest != 'break_page') && ($type_quest != 'title')) {
            ++$quest_sequence_number;
            $quetion[$i]['idQuest'] = $idQuest;
            $quetion[$i]['title'] = getQuestionInfo($idQuest);
            $i = $i + 1;
        }
    }
    $result = array();
    $i = 0;
    foreach ($valid_track as $id_track) {
        $query_userId = "SELECT id_user FROM " . $GLOBALS['prefix_lms'] . "_polltrack WHERE id_track = " . $id_track;
        $result_userId = sql_query($query_userId);
        while (list($id_user) = sql_fetch_row($result_userId)) {
            $result[$i]['idUser'] = $id_user;
            $result[$i]['name'] = getFullName($id_user);
        }
        $answer_quest = array();
        foreach ($quetion AS $q) {
            $query_answer = "SELECT id_answer FROM " . $GLOBALS['prefix_lms'] . "_polltrack_answer WHERE id_track = " . $id_track . " AND id_quest = " . $q['idQuest'];
            $result_answer = sql_query($query_answer);
            $answer_detail = '';
            $num_row = sql_num_rows($result_answer);
            if ($num_row < 2) {
                while (list($id_answer) = sql_fetch_row($result_answer)) {
                    if ($answer_detail === '') {
                        $answer_detail = getAnswerInfo($id_answer, $q['idQuest'], $id_track);
                        $answer_quest[] = $answer_detail;
                    } else {
                        $answer_detail = $answer_detail . ' - ' . getAnswerInfo($id_answer, $q['idQuest'], $id_track);
                        $answer_quest[] = $answer_detail;
                    }
                }
            }else{
                while (list($id_answer) = sql_fetch_row($result_answer)) {
                    $multil_answer[] = $id_answer;
                }
                $answer_detail = '';
                foreach ($multil_answer AS $a){
                    if ($answer_detail === '') {
                        $answer_detail = getAnswerInfo($a, $q['idQuest'], $id_track);
                    } else {
                        $answer_detail = $answer_detail . ' - ' . getAnswerInfo($a, $q['idQuest'], $id_track);
                    }
                }
                $answer_quest[] = $answer_detail;
            }
        }
        $result[$i]['answer'] = $answer_quest;
        $i = $i + 1;
    }
...
//        echo '</pre>';
        $csv = '';
        $csv = $csv . renderQuestion($quetion);
        $csv = $csv . "\n";
        $csv = $csv . renderCSV($result);
        sendStrAsFile($csv, $filename);
//        sendStrAsFile($filetext, $filename);
    }

    $treeview_value = str_replace('treeview_selected_' . $_SESSION['idCourse'], '', array_search($poll_info['title'], $_POST));

    $str = Form::openForm('tree_export_form', 'index.php?modname=stats&amp;op=statcourse')
            . Form::getHidden('seq_0.' . $treeview_value, 'treeview_selected_' . $_SESSION['idCourse'] . $treeview_value, $poll_info['title'])
            . Form::getHidden('treeview_selected_' . $_SESSION['idCourse'], 'treeview_selected_' . $_SESSION['idCourse'], $treeview_value)
            . Form::getHidden('treeview_state_' . $_SESSION['idCourse'], 'treeview_state_' . $_SESSION['idCourse'], $_POST['treeview_state_' . $_SESSION['idCourse']])
            . Form::openButtonSpace()
            ...
            . Form::closeForm()
    ;
    $roll = Docebo::user()->getUserLevelId();
    if ($mvc) {
        if ($roll === '/framework/level/godadmin') {
            $output .= $str;
        }
    } else {
        if ($roll === '/framework/level/godadmin') {
...
function getFullName($id_user) {
    $sql = "SELECT firstname, lastname FROM core_user WHERE idst = " . $id_user;
    $result = sql_query($sql);
    list($firstname, $lastname) = sql_fetch_row($result);
    return $lastname . ' ' . $firstname;
}

function getQuestionInfo($idQuest) {
    $sql = "SELECT title_quest FROM " . $GLOBALS['prefix_lms'] . "_pollquest WHERE id_quest = " . $idQuest;
    $result = sql_query($sql);
    list($title_quest) = sql_fetch_row($result);
    return $title_quest;
}

function getAnswerInfo($id_answer, $id_quest, $id_track) {
    if ($id_answer != 0) {
        $query = "SELECT answer FROM " . $GLOBALS['prefix_lms'] . "_pollquestanswer WHERE id_answer = " . $id_answer;
        $result = sql_query($query);
        list($answer) = sql_fetch_row($result);
        return $answer;
    } else {
        $query = "SELECT more_info FROM " . $GLOBALS['prefix_lms'] . "_polltrack_answer WHERE id_answer = " . $id_answer . " AND id_quest = " . $id_quest . " AND id_track = " . $id_track;
        $result = sql_query($query);
        list($more_info) = sql_fetch_row($result);
        return $more_info;
    }
}
