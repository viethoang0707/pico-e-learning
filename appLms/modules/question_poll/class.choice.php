...
class Choice_QuestionPoll extends QuestionPoll {
    ...
    function create($id_poll, $back_poll) {
        ...
        $GLOBALS['page']->add(
                ...
                . Form::getTextarea($lang->def('_POLL_QUEST_TITLE'), 'title_quest', 'title_quest', ( isset($_POST['title_quest']) ? stripslashes($_POST['title_quest']) : ''))
                . Form::getDropdown($lang->def('_CATEGORY'), 'id_category', 'id_category', $categories, ( isset($_POST['id_category']) ? $_POST['id_category'] : $lang->def('_POLL_QUEST_TITLE')))
    ...
    function edit($back_poll) {
        ...
        $GLOBALS['page']->add(
                ...
                . Form::getHidden('back_poll', 'back_poll', $url_encode)
                . Form::getHidden('num_answer', 'num_answer', $num_answer)
                . Form::getTextarea($lang->def('_POLL_QUEST_TITLE'), 'title_quest', 'title_quest', ( isset($_POST['title_quest']) ? stripslashes($_POST['title_quest']) : $quest))
                . Form::getDropdown($lang->def('_CATEGORY'), 'id_category', 'id_category', $categories, ( isset($_POST['id_category']) ? $_POST['id_category'] : $sel_cat))
                . '<div class="nofloat"></div><br />', 'content');
    ...
    function play($num_quest, $shuffle_answer = false, $id_track = 0, $freeze = false) {
        ...
        if ($shuffle_answer)
            ...
            $query_answer .= " ORDER BY sequence";
    ...
    function playReport($num_quest, $tot_tracks, &$valid_track) {
        ...
        if (is_array($valid_track) && !empty($valid_track))
            $recover_answer .= " AND id_track IN ( " . implode(',', $valid_track) . " ) ";
        ...
        if (Get::sett('no_answer_in_test') == 'on') {
            $content .= '<tr><td>' . $lang->def('_NO_ANSWER') . '</td><td>'
                    . ( $max != 0 ? '<div class="colored_row" style="width: ' . round($not_answer / $max * 90, 2) . '%;">' . ( $not_answer != 0 ? $not_answer : '' ) . '</div>' : '' )
                    . '</td></tr>';
        }
        $content .= '</table>'
                . '</div>'
    ...
    function export_CSV_Poll($num_quest, $tot_tracks, &$valid_track) {
        $lang = & DoceboLanguage::createInstance('poll');
        ...
        while (list($id_answer, $answer) = sql_fetch_row($re_answer)) {
            $text = '';
            $query = "SELECT p.id_user FROM " . $GLOBALS['prefix_lms'] . "_polltrack AS p JOIN " . $GLOBALS['prefix_lms'] . "_polltrack_answer AS pa ON pa.id_track = p.id_track WHERE pa.id_answer = " . $id_answer;
            ...
            while (list($id_user) = sql_fetch_row($result)) {
                ...
                $text .= $this->getFullName($id_user);
                $result_final[$i]['name'] = $this->getFullName($id_user);
                $result_final[$i]['answer'] = $answer;
                $i = $i + 1;
            }

            if (isset($num_answer[$id_answer]) && $max != 0) {
                $content .= ';"' . str_replace('"', '""', $answer) . '";' . $num_answer[$id_answer] . ';' . $text . "\r\n"; //.',"'.round(($num_answer[$id_answer] / $max) * 100, 2).'%"'
            } else {
                $content .= ';"' . str_replace('"', '""', $answer) . '";0' . "\r\n";
            }
        }
