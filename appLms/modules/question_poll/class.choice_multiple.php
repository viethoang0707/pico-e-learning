...
class ChoiceMultiple_QuestionPoll extends QuestionPoll {
    ...
    function create($id_poll, $back_poll) {
        ...
        $GLOBALS['page']->add(getTitleArea($lang->def('_POLL_SECTION'), 'poll')
                ...
                . Form::getTextarea($lang->def('_POLL_QUEST_TITLE'), 'title_quest', 'title_quest', ( isset($_POST['title_quest']) ? stripslashes($_POST['title_quest']) : $lang->def('_POLL_QUEST_TITLE')))
                . Form::getDropdown($lang->def('_CATEGORY'), 'id_category', 'id_category', $categories, ( isset($_POST['id_category']) ? $_POST['id_category'] : ''))
    ...
    function export_CSV_Poll($num_quest, $tot_answer, &$valid_track) {
        $lang = & DoceboLanguage::createInstance('poll');
        ...
        while (list($id_answer, $answer) = sql_fetch_row($re_answer)) {
            ...
            if (isset($num_answer[$id_answer]) && $max != 0) {
                ...
                $content .= ';"' . str_replace('"', '""', $answer) . '";0' . '";' . $text . "\r\n";
            }
        }
        return $content;
    }
