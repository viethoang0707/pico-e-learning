...
class ExtendedText_QuestionPoll extends QuestionPoll {
    ...
    function export_CSV_Poll($num_quest, $tot_tracks, &$valid_track) {
        ...
        $query_answer = "SELECT more_info, id_track" .
                " FROM " . $GLOBALS['prefix_lms'] . "_polltrack_answer" .
                " WHERE id_quest = '" . $id_quest . "'";
        ...
        while (list($more_info, $id_track) = sql_fetch_row($result_answer)) {
            $query_name = "SELECT id_user FROM " . $GLOBALS['prefix_lms'] . "_polltrack WHERE id_track = " . $id_track;
            $result = sql_query($query_name);
            list($id_user) = sql_fetch_row($result);
            $name = $this->getFullName($id_user);
            $csv .= ';"' . str_replace('"', '""', $more_info) . '";;' . $name . ';' . "\r\n";
        }
        return $csv;
    }
