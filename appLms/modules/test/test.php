...
function delquest() {
    ...
    if (isset($_GET['confirm'])) {
        ...
        $GLOBALS['page']->add(
                '<div class="std_block">'
                . getDeleteUi($lang->def('_AREYOUSURE'), '<span class="text_bold">' . $lang->def('_TYPE') . ' : </span>'
                        . $lang->def('_QUEST_ACRN_' . strtoupper($type_quest)) . ' - ' . $lang->def('_QUEST_' . strtoupper($type_quest)) . '<br />'
                        . '<span class="text_bold">' . $lang->def('_QUESTION') . ' : </span>' . $title_quest, true, 'index.php?modname=test&amp;op=delquest&amp;idQuest=' . $idQuest . '&amp;back_url=' . $url_coded . '&amp;confirm=1', 'index.php?modname=test&amp;op=modtestgui&amp;idTest=' . $idTest . '&amp;back_url=' . $url_coded
                )
                . '</div>', 'content');
    }
}
    ...
    if ($_POST['order_type'] == 3) {
        ...
        if (isset($_POST['question_random_category']) && is_array($_POST['question_random_category'])) {
            foreach ($_POST['question_random_category'] as $key => $value) {
                if ((int) $value > 0)
                    $arr[] = array('id_category' => $key, 'selected' => (int) $value);
            }
        }
        ...
        switch ($_POST['time_limit']) {
            ...
            case 1 : {
                    ...
                    $GLOBALS['page']->add(
                            ...
                            . Form::getTextfield($lang->def('_TEST_TM2_NEWTIMETOTAL'), 'time_assigned', 'time_assigned', 5, $time_assigned, $lang->def('_TEST_TM2_NEWTIMETOTAL'), $lang->def('_SECONDS'))
                            . Form::getCloseFieldset()
                            . Form::openButtonSpace()
            ...
            case 2 : {
				...
				WHERE idTest = '$idTest'"));
                    ...
                    $GLOBALS['page']->add(
                            ...
                            . Form::getTextfield($lang->def('_TEST_TM2_NEWTIME'), 'new_time', 'new_time', 10, $actual_tot_time, $lang->def('_TEST_TM2_NEWTIME'), $lang->def('_SECONDS'))
...
function modassigntime() {
    ...
    while (list($idQuest, $type_quest, $title_quest, $difficult, $time_assigned) = sql_fetch_row($re_quest)) {
        ...
        $content = array(
            ...
            ( $difficult ?
                    ...
                    . Form::getInputDropdown('', 'new_difficult_quest_' . $idQuest, 'new_difficult_quest[' . $idQuest . ']', array(1 => 1, 2, 3, 4, 5), $difficult, '') :
                    '&nbsp;' ),
            ( $difficult ?
    ...
    if (isset($_GET['new_time']))
        ...
        $previous_time = $previous_time = $_POST['previous_time'];
...
function defpoint() {
    ...
    $GLOBALS['page']->add(
            ...
            . Form::getHidden('idTest', 'idTest', $idTest)
            . Form::getHidden('back_url', 'back_url', $url_coded)
            . Form::getTextfield($lang->def('_TEST_PMM_REQUIREDSCORE_POINT'), 'point_required', 'point_required', 30, ( $point_required ? $point_required : '0.0'))
            . Form::getCloseFieldset()
    ...
    $GLOBALS['page']->add(
            ...
            . Form::getTextfield($lang->def('_TEST_QUEST_NEWMAXTESTSCORE'), 'new_assigned_score', 'new_assigned_score', 255, $max_score, $lang->def('_TEST_QUEST_NEWMAXTESTSCORE'), $lang->def('_SCORE'))
...
if ($_POST['export_quest_select'] == 5)
