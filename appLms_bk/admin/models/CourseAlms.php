...
Class CourseAlms extends Model
{
	...
	public function insCourse()
	{
		...
		if ($_POST['course_type']=='classroom') {
			$this->setupConference($id_course,$_POST['course_name'],$date_begin.' '.$hour_begin,$date_end.' '.$hour_end);
		}
	...
	private function setupConference($id_course,$name,$start_date,$end_date) {
		require_once($GLOBALS['where_scs'].'/lib/lib.conference.php');
		$start_timestamp = strtotime($start_date);
		$end_timestamp = strtotime($end_date);
		$logoutUrl = 'appLms/index.php?modname=organization&op=custom_playitem';
		$conference = new Conference_Manager();
		$conference->insert_room($id_course,0,$name,'bbb',$start_timestamp,$end_timestamp,0,9999,true, 0, 0, 0,$logoutUrl);
	}
