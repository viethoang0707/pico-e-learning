...
Class SubscriptionAlms extends Model
{
	...
	public function subscribeUser($id_user, $level, $waiting, $date_begin_validity = FALSE, $date_expire_validity = FALSE) {
		if($this->id_edition != 0) {
			...
			require_once(_lms_.'/lib/lib.subscribe.php');
			$subscribe_man = new CourseSubscribe_Manager();
			if(!$subscribe_man->controlSubscription($id_user, $this->id_course)) {
				$this->setupConferenceMember($id_user,$level);
			}
			$date_man = new DateManager();
	...
	private function setupConferenceMember($id_user,$level) {
    	...
    	$message = $role.' '.$id_user.' '.$user_info[ACL_INFO_USERID] . $user_info[ACL_INFO_FIRSTNAME].' '.$user_info[ACL_INFO_LASTNAME].' '.$user_info[ACL_INFO_EMAIL];
    	error_log ($message);
		$conference = new Conference_Manager();
		$memberId = $conference->insert_room_member($this->id_course,$user_info[ACL_INFO_USERID],
			$user_info[ACL_INFO_FIRSTNAME].' '.$user_info[ACL_INFO_LASTNAME],$user_info[ACL_INFO_EMAIL],
			'1234',array(),$role);		
	}
