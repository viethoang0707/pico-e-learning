...
class PublicRepoDirDb extends TreeDb {//RepoDirDb
...
class PublicRepoTreeView extends TreeView {//RepoTreeView
	var $options = array();
	...
	*/
	...
	function LOSelector( $module, $back_url ) {
		...
		while( list($objectType) = sql_fetch_row($rs) ) {
			...
			$out .=  '<label for="'.$objectType.'">'. Lang::t('_LONAME_'.$objectType).'</label>'
				.'<br />';
			$first = FALSE;
		}
		$out .=  $this->printState();
		$out .=  '<br /><input type="submit" class="button" value="'. Lang::t('_REPOCREATELO')
