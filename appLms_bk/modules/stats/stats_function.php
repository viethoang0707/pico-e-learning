...
function getAllAnswerOfCourse($idUser, $idCourse, $idOrg) {
    $answer = array();
    $query = "SELECT track.idTrack FROM learning_testtrack AS track JOIN learning_courseuser AS user ON track.idUser = user.idUser AND track.score_status = 'valid' AND track.idUser = '" . $idUser . "' AND user.idCourse = '" . $idCourse . "' JOIN learning_organization AS org ON track.idReference = org.idOrg AND org.idOrg ='" . $idOrg . "'";
    $rs = sql_query($query);
...
function getTestTrackAnswer($idTrack) {
    $query = "SELECT * FROM learning_testtrack_answer WHERE idTrack ='" . $idTrack . "' AND score_assigned != 0 OR manual_assigned !=0 GROUP BY idQuest";
    $rs = sql_query($query);
    $count = 0;
    while (list($values) = sql_fetch_row($rs)) {
        echo $count;
        $count = $count + 1;
    };
    return $count;
...
function getMinMarkOfTest($idCourse) {
    ...
    if ($test < 1) {
        ...
        $mark[0]['mark'] = getMinMark($idOrg, $idCourse);
        $mark[0]['test'] = getTestName($idOrg);
    } else {
        $i = 0;
        while (list($idOrg) = sql_fetch_row($rs)) {
            $mark[$i]['mark'] = getMinMark($idOrg, $idCourse);
            $mark[$i]['test'] = getTestName($idOrg);
            $i = $i + 1;
        }
    }
    return $mark;
...
function getMaxMarkOfTest($idCourse) {
    ...
    if ($test < 1) {
        list($idOrg) = sql_fetch_row($rs);
        $mark[0]['mark'] = getMaxMark($idOrg, $idCourse);
        $mark[0]['test'] = getTestName($idOrg);
    } else {
        $i = 0;
        while (list($idOrg) = sql_fetch_row($rs)) {
            $mark[$i]['mark'] = getMaxMark($idOrg, $idCourse);
            $mark[$i]['test'] = getTestName($idOrg);
            $i = $i + 1;
        }
    }
...
function getScoreStudentOfCourse($idUser, $idCourse, $idOrg) {
    $answer = array();
    $query = "SELECT * FROM learning_testtrack AS track JOIN learning_courseuser AS user ON track.idUser = user.idUser AND track.score_status = 'valid' AND track.idUser = '" . $idUser . "' AND user.idCourse = '" . $idCourse . "' JOIN learning_organization AS org ON track.idReference = org.idOrg AND org.idOrg ='" . $idOrg . "'";
    $rs = sql_query($query);
...
function getTestName($idOrg) {
    $query = "SELECT title FROM learning_organization WHERE idOrg = '" . $idOrg . "'";
    list($title) = sql_fetch_row(sql_query($query));
    return $title;
}
