...
class Bbb_Manager {
	...
	function _getRoomMemberTable() {

		return $GLOBALS['prefix_scs'].'_rules_user';
	}
...
*/
	...
	function getUrl($idConference,$room_type) {
		...
		$meetingId = $room['confkey'];

		$query3="SELECT * FROM ".$this->_getRoomMemberTable()." WHERE id_room = '".$idConference."'"
		." AND userid = '".$u_info[ACL_INFO_USERID]."'";
		$re_member = $this->_query($query3);
		$member=$this->nextRow($re_member);
		$memberId =$member['picture'];

		error_log('Member'.$memberId.$idConference.' '.$u_info[ACL_INFO_USERID]);
		
		$server= Get::sett('bbb_server', "");
    	$salt = Get::sett('bbb_salt', "");
    	$port = Get::sett('bbb_port', "");

    	require_once(_base_.'/lib/lib.json.php');
		$json = new Services_JSON();
    	
    	$payload = array('meetingId'=>$meetingId,'memberId'=>$memberId);
		$payload_encode = $json->encode($payload);
		$checksum = sha1($payload_encode.$salt);

		$_url = Get::sett('bbb_user', "").'/#/trustedlogin?payload='.base64_encode($payload_encode).'&checksum='.$checksum;



////////////////////////////////////////////////////
		$url = '<a onclick="window.open(this.href, \'\', \'\');return false;" href="'.str_replace('&', '&amp;', $_url).'">'.'Enter'.'</a>';
	...
	function getRoomInfo($idConference) {
	...
	function openMeeting($name,$ref,$welcome,$duration,$logoutUrl,$data,$active) {
		require_once(_base_.'/lib/lib.json.php');
		require_once(_base_.'/lib/lib.fsock_wrapper.php');
		$fsock = new FSock('https');
		$json = new Services_JSON();
		$server= Get::sett('bbb_server', "");
    	$salt = Get::sett('bbb_salt', "");
    	$port = Get::sett('bbb_port', "");
		$url = $server.':'.$port.'/api/trusted/meeting';

		$exit_url=base64_encode( Get::sett('bbb_password_moderator', "").'/'.$logoutUrl);
		error_log(Get::sett('bbb_password_moderator', ""));
		error_log($exit_url);
		...
		if(curl_errno($ch)){
			$output = "<script>console.log( 'Debug Openmeeting: " . curl_error($ch) . "' );</script>";
   			echo $output;
		}
		curl_close($ch);

		if ($res_json) {
			$data = $json->decode($res_json);
			return $data->id;
		}
		return null;
	}

	function addMeetubgMember($roomId,$name,$email,$password,$data,$role) {
		require_once(_base_.'/lib/lib.json.php');
		require_once(_base_.'/lib/lib.fsock_wrapper.php');
		$fsock = new FSock('https');
		$json = new Services_JSON();
    	...
    	$salt = Get::sett('bbb_salt', "");
    	$port = Get::sett('bbb_port', "");
		$url = $server.':'.$port.'/api/trusted/member';
		$conference = $this->getRoomInfo($roomId);
