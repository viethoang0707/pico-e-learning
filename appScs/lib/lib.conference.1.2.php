...
class Conference_Manager {
	...
	function insert_room_member($idCourse,$userid,$name,$email,$password,$data,$role) {
		$room = $this->roomActive($idCourse);
		if ($room) {
			$bbb = new Bbb_Manager();
			$memberId = $bbb->addMeetubgMember($room['id'],$name,$email,$password,$data,$role);
			$insert_room_member = "
			INSERT INTO ".$this->_getRoomMemberTable()."
				( id_room,userid,picture,first_name) VALUES (
					'".(int)$room['id']."',
					'".$userid."',
					'".$memberId."',
					'".$name."'
				)";
			error_log($insert_room_member);
			$ok=true;
			if(!sql_query($insert_room_member)) $ok=false;
			if ($ok) $iduser=sql_insert_id();
			return $ok;
		}
		return False;
	}

	function insert_room($idCourse,$idSt,$name,$room_type,$start_timestamp,$end_timestamp,$meetinghours,$maxparticipants,$bookable, $startdate, $starthour, $startminute,$logoutUrl) {
...
// TODO : support for BBB is experimental - must be refined
					...
					$confkey = $bbb->openMeeting($name,'e-Training','',60,$logoutUrl,array(),true);
	...
	function roomActive($idCourse, $at_date = false) {
		...
		$query = "SELECT id,idCourse,idSt,name,room_type,starttime,endtime,meetinghours,maxparticipants,bookable
		FROM ".$this->_getRoomTable()."	WHERE idCourse = '".$idCourse."'";
