...
class UserProfile {
    ...
    function enableModViewerPolicy() {
        $this->_policy_mod = true;
    }

    function disableModViewerPolicy() {
    ...
    function getAction() {
        ...
        if (isset($_REQUEST[$this->_varname_action]))
            return Get::req($this->_varname_action, DOTY_MIXED, '');
        else
            return 'profileview';
    }
...
class UserProfileViewer {
    ...
    function resolveUsername($name_only = false) {
        ...
        if ($name_only) {
            ...
            return ( $this->user_info[ACL_INFO_FIRSTNAME] ? $this->user_info[ACL_INFO_FIRSTNAME] : $this->acl_man->relativeId($this->user_info[ACL_INFO_USERID]) );
        } else {

            return ( $this->user_info[ACL_INFO_LASTNAME] . $this->user_info[ACL_INFO_FIRSTNAME] ? $this->user_info[ACL_INFO_LASTNAME] . ' ' . $this->user_info[ACL_INFO_FIRSTNAME] : $this->acl_man->relativeId($this->user_info[ACL_INFO_USERID]) );
        }
    }
    ...
    function getAvailablePhotoAvatar($dimension = 'medium') {
        ...
        if ($this->user_info[ACL_INFO_AVATAR] != "") {
            return $this->getPASrc($this->user_info[ACL_INFO_AVATAR], $this->_lang->def('_AVATAR'), $class_picture);
        }
    ...
    function getUserInfo() {
        ...
        if ($this->_user_profile->useAvatar()) {
            ...
            $html .= ( $edit_mode ? '<a href="' . $this->_url_man->getUrl($this->_varname_action . '=uploadavatar') . '" title="' . $this->_lang->def('_MOD') . '">' : '' )
                    . ( ($this->user_info[ACL_INFO_AVATAR] != "") ? $this->getPASrc($this->user_info[ACL_INFO_AVATAR], $this->_lang->def('_AVATAR'), $class_picture) : '<img src="' . getPathImage() . 'standard/user.png" alt="' . $this->_lang->def('_NOAVATAR') . '" />' )
                    . '<br />'
        ...
        $html .= $this->getUIRowCode($this->_lang->def('_USERNAME'), $this->acl_man->relativeId($this->user_info[ACL_INFO_USERID]))
                . $this->getUIRowCode($this->_lang->def('_LASTNAME'), ( $this->user_info[ACL_INFO_LASTNAME] !== false ? $this->user_info[ACL_INFO_LASTNAME] : $this->_lang->def('_HIDDEN')))
                . $this->getUIRowCode($this->_lang->def('_FIRSTNAME'), ( $this->user_info[ACL_INFO_FIRSTNAME] !== false ? $this->user_info[ACL_INFO_FIRSTNAME] : $this->_lang->def('_HIDDEN')));
        ...
        if ($this->_user_profile->godMode()) {
            ...
            switch ($acl_man->getUserLevelId($this->_user_profile->getIdUser())) {
                case ADMIN_GROUP_GODADMIN : $user_level_string = $lv_lang->def('_DIRECTORY_' . ADMIN_GROUP_GODADMIN);
                    break;
                case ADMIN_GROUP_ADMIN : $user_level_string = $lv_lang->def('_DIRECTORY_' . ADMIN_GROUP_ADMIN);
                    break;
                case ADMIN_GROUP_PUBLICADMIN: $user_level_string = $lv_lang->def('_DIRECTORY_' . ADMIN_GROUP_PUBLICADMIN);
                    break;
                case ADMIN_GROUP_USER : $user_level_string = $lv_lang->def('_DIRECTORY_' . ADMIN_GROUP_USER);
                    break;
        ...
        if ($viewer == $this->_id_user) {
            ...
            $html .= '<tr><th scope="row" id="up_quota">' . $this->_lang->def('_USER_QUOTA') . '</th>'
                    ...
                    . ( $max_quota == USER_QUOTA_UNLIMIT ? ' ' . $actual_size . ' MB / ' . $this->_lang->def('_UNLIMITED_QUOTA') . ' ' : Util::draw_progress_bar($percent, true, 'progress_bar up_quota_bar', false, false)
                    )
                    . '</td></tr>';
        }
        ...
        $html .= '<tr><th scope="row" id="up_email">' . $this->_lang->def('_EMAIL') . '</th>'
                . '<td>'
                . ( $this->user_info[ACL_INFO_EMAIL] !== false ? '<a href="mailto:' . $this->user_info[ACL_INFO_EMAIL] . '">' . $this->user_info[ACL_INFO_EMAIL] . '</a>' : $this->_lang->def('_HIDDEN') )
                . '</td></tr>';
            ...
            case "video" : {
                    ...
                    if ($re_files && $num_files > 0) {
                        ...
                        while ($video = $user_file->fetch_row($re_files)) {
                            ...
                            $class_limit = '';
                            $html .= '<li>'
                                    . ( $play_code === false ? implode('_', array_slice(explode('_', $video[MYFILE_FILE_NAME]), 3)) : $play_code )
                                    . '<br />'
            ...
            case "audio" : {
                    ...
                    if ($re_files && $num_files > 0) {
                        ...
                        while ($audio = $user_file->fetch_row($re_files)) {
                            ...
                            $class_limit = '';
                            $html .= '<li>'
                                    . ( $play_code === false ? implode('_', array_slice(explode('_', $audio[MYFILE_FILE_NAME]), 3)) : $play_code )
                                    . '<br />'
    ...
    function getViewUserFileDetail() {
        ...
        $id_thread = Get::req('idThread', DOTY_INT, 0);
        $html .= getBackUi($this->_url_man->getUrl('id_user=' . $this->_id_user . '&' . $this->_varname_action . '=view_files&type=' . $type . '&idThread=' . $id_thread . ''), $this->_lang->def('_BACK'));
    ...
    function tinyUserInfo($picture) {
        ...
        $html .=( ($this->user_info[ACL_INFO_AVATAR] != "") ? $this->getPASrc($this->user_info[ACL_INFO_AVATAR], $this->_lang->def('_AVATAR'), $class_picture) : '<img class="' . $class_picture . '" src="' . getPathImage() . 'standard/user.png" alt="' . $this->_lang->def('_NOAVATAR') . '" />' )
                . '<br />'
                . $this->_lang->def('_AVATAR');
        // end avatar --------------------------------------------------------------------------
        $html .= '</div>';
        ...
        $html .= $this->getUIRowCode($this->_lang->def('_USERNAME'), $this->acl_man->relativeId($this->user_info[ACL_INFO_USERID]))
                ...
                . $this->getUIRowCode($this->_lang->def('_EMAIL'), ( $this->user_info[ACL_INFO_EMAIL] !== false ? '<a href="mailto:' . $this->user_info[ACL_INFO_EMAIL] . '">' . $this->user_info[ACL_INFO_EMAIL] . '</a>' : $this->_lang->def('_HIDDEN')));
        // user extra field ------------------------------------------------------------------
    ...
    function minimalUserInfo($picture, $link_to = false) {
        ...
        $online_status = ( $is_online === 'unk' ? $this->_lang->def('_HIDDEN') : ( $is_online ? $this->_lang->def('_USERONLINE') : $this->_lang->def('_USEROFFLINE') )
                );
    ...
    function homeUserProfile($picture = false, $viewer = false, $intest = false) {
        ...
        if ($access_career) {
            ...
            $html .= '<div class="inline_block">'
                    ...
                    . ( isset($course_stats['with_ustatus'][_CUS_END]) && $course_stats['with_ustatus'][_CUS_END] != 0 ? '<tr><th scope="row">' . $this->_lang->def('_COURSE_END') . ' :</th><td>' . $course_stats['with_ustatus'][_CUS_END] . '</td></tr>' : '' )
                    . ( isset($course_stats['expiring']) && $course_stats['expiring'] != 0 ? '<tr><th scope="row">' . $this->_lang->def('_COURSE_EXPIRING') . ' :</th><td>' . $course_stats['expiring'] . '</td></tr>' : '' );
            ...
            $html .= ''
                    . ( isset($course_stats['cert_relesable']) && $tot_cert != 0 ? '<tr><th scope="row">' . $this->_lang->def('_CERT_RELESABLE') . ' :</th><td><a href="index.php?modname=mycertificate&amp;op=mycertificate">' . $tot_cert . '</a></td></tr>' : '' )
                    . ( $pendent != 0 ? '<tr><th scope="row">' . $this->_lang->def('_FRIEND_PENDENT') . ' :</th><td><a href="index.php?modname=myfriends&amp;op=myfriends">' . $pendent . '</a></td></tr>' : '' )
                    . '</table>'
                    . '</div>'
                    . '</div>'
                    . '</div>';
    ...
    function getUserPanelData($picture) {
        ...
        $data['avatar'] = ( ($this->user_info[ACL_INFO_AVATAR] != "") ? $this->getPASrc($this->user_info[ACL_INFO_AVATAR], $this->_lang->def('_AVATAR'), $class_picture) : '<img class="' . $class_picture . '" src="' . getPathImage() . 'standard/user.png" alt="' . $this->_lang->def('_NOAVATAR') . '" />' );
    ...
    function getUserInfoModUi() {
        ...
        $html .= Form::getTextfield($this->_lang->def('_LASTNAME'), 'up_lastname', 'up_lastname', '255', Get::req('up_lastname', DOTY_MIXED, $this->user_info[ACL_INFO_LASTNAME], true))
                . Form::getTextfield($this->_lang->def('_FIRSTNAME'), 'up_firstname', 'up_firstname', '255', Get::req('up_firstname', DOTY_MIXED, $this->user_info[ACL_INFO_FIRSTNAME], true))
                . Form::getTextfield($this->_lang->def('_EMAIL'), 'up_email', 'up_email', '255', Get::req('up_email', DOTY_MIXED, $this->user_info[ACL_INFO_EMAIL], true));
        // user extra field ------------------------------------------------------------------
    ...
    function modUserPolicyGui() {
        ...
        $html .= $this->getUIPolicyCode($this->_lang->def('_PRIVATE_MESSAGE_FROM'), false, Form::getInputDropdown('dropdown_wh', 'policy_selected_message_recipients', 'policy_selected[message_recipients]', $reduced_policy_arr, (isset($field_policy['message_recipients']) ? $field_policy['message_recipients'] : /* PFL_POLICY_TEACHER_AND_FRIENDS */PFL_POLICY_FREE), ''
                )
        );
        $html .= $this->getUIPolicyCode($this->_lang->def('_SHOWME_ONLINE'), false, Form::getInputDropdown('dropdown_wh', 'policy_selected_online_satus', 'policy_selected[online_satus]', $reduced_policy_arr, (isset($field_policy['online_satus']) ? $field_policy['online_satus'] : /* PFL_POLICY_TEACHER_AND_FRIENDS */PFL_POLICY_FREE), ''
                )
        );
        ...
        if (!empty($friend_list) && is_array($friend_list)) {
            ...
            while ((list($id, $info) = each($friend_list)) && $i < 7) {
                ...
                $html .= '<li>'
                        ...
                        . ( $info[ACL_INFO_AVATAR] != "" ? $this->getPASrc($info[ACL_INFO_AVATAR], $this->_lang->def('_AVATAR'), 'image_limit_small') : '<img class="image_limit_small" src="' . getPathImage() . 'standard/user.png" alt="' . $this->_lang->def('_NOAVATAR') . '" />' )
                        . $friend_username
                        . '</a></li>';
                $i++;
            }
    ...
    function delTeacherCurriculumGui() {
        if (isset($_GET['confirm'])) {
            ...
            $html .= getDeleteUi(
                    $this->_lang->def('_AREYOUSURE'), $this->_lang->def('_NAME'), true, $this->_url_man->getUrl('ap=del_teach_curric&confirm=1'), //'index.php?modname=profile&op=profile&ap=del_teach_curric&confirm=1',
    ...
    function modTeacherCurriculumGui() {
        ...
        $html .= Form::getTextarea($this->_lang->def('_TEACHER_CURRICULUM'), 'tp_curriculum', 'tp_curriculum', Get::req('tp_curriculum', DOTY_MIXED, $this->_up_data_man->getTeacherCurriculum($this->_user_profile->getIdUser())));
...
class UserProfileData {
     ...
     */
    function getPAPath() {
        return '/appCore/' . Get::sett('pathphoto');
    }
    ...
    function getVisibilityResponse($rule, $id_user, $viewer, $is_teacher, $is_friend) {
        ...
        if ($id_user == $viewer)
            return true;
        switch ($rule) {
            case PFL_POLICY_FREE : return true;
                break;
    ...
    function canSendMessage($id_user, $sender) {
        ...
        switch ($fal['message_recipients']) {
            case PFL_POLICY_FREE : $can_send = true;
                break;
    ...
    function saveUserPwd($id_user, $new_pwd) {
        ...
        if (!$this->acl_man->updateUser($id_user, FALSE, false, false, $new_pwd, false, FALSE, false)) {
            return false;
        }
        return true;
    }
    ...
    function getFileInfo($id_user, $viewer) {
        ...
        $files_info = array();
        require_once($GLOBALS['where_framework'] . '/lib/lib.myfiles.php');
        $user_file = new MyFilesPolicy($this->_user_profile->getIdUser(), $this->getViewer(), $is_friend, $is_teacher);
    ...
    function saveTeacherCurriculumAndPublication($id_user, $curriculum = false, $publications = false) {
        ...
        if ($num_of) {
			...
			SET " . ( $curriculum !== false ? " curriculum = '" . $curriculum . "' " . ($publications !== false ? ", " : "" ) : "" )
                    . ( $publications !== false ? " publications = '" . $publications . "' " : "" ) . "
			WHERE id_user = '" . $id_user . "'";
            $re = $this->_query($query);
        } else {
